# 项目状态板示例

**项目 ID**: ADF-BOOTSTRAP-001
**项目名称**: AgentDevFlow 中文执行层收口
**日期**: 2026-04-09
**负责人**: 团队负责人

## 当前阶段

- 治理 / 内化收口

## 活跃 Issue

- #0 仓库主线整理
- 多个流程治理子议题以 `prompts/discuss/00x` 形式并行收口

## 阻塞项

- 平台自动化仍是样例和模板为主，尚未形成真实 CI / Bot / Hook 方案

## 当前阶段门队列

- 工作流与模板绑定复核
- 平台检查失败场景补强

## 人工评审队列

- 共享角色执行手册完整性复核
- README 与目录结构可用性复核

## 平台检查状态

- Claude: checklist ready / result example ready
- Codex: checklist ready / result example ready
- OpenCode: checklist ready / result example ready

## 待办概览

- TODO-20260409-001 补齐 启动会 示例资产
- TODO-20260409-002 补工作流与模板绑定示例

## 下一步动作

- 继续补平台自动化样例
- 继续补 Issue / 评论 / 产物关联失败拦截链路
