---
name: adf-pmo-review
description: PMO 系统化地逐个解决 `docs/adf-pmo/issues/` 下的 Open 问题：分析根因、与用户对齐方案、推动机制改进落地。
user-invocable: true
---
...
PMO 系统化地逐个解决 `docs/adf-pmo/issues/` 下的 Open 问题：分析根因、与用户对齐方案、推动机制改进落地。
...
读取 `docs/adf-pmo/issues/` 下的所有 Open 问题文件（扁平结构）：
...
文件路径：`docs/adf-pmo/resolutions/{ID}_{日期}_{简短描述}_resolution.md`
...
变更完成后，更新：
- `docs/adf-pmo/issues/README.md`
- `docs/adf-pmo/resolutions/README.md`
...
```bash
git add docs/adf-pmo/issues/ docs/adf-pmo/resolutions/ {涉及的其他文件}
