...
- Human Review #1 通过后，推动 Human 合并文档 PR（`doc-{issue}` 分支）
- Human Review #2 通过后，推动 Human 合并代码 PR（`feature-{issue}` 分支）
...
| 文档 PR 合并 | 确认 HR#1 评审完成、推动 Human 合并 | 代替 Human 做合并决策 | Human 合并后通知 Engineer 开始实现 |
...
| Gate 5 | 主持 Release 评审、推动 Human 发布 | 代替 Human 做发布决策 | Human 发布后关闭 Issue |
...
- 不得代替 Architect / QA / 平台与发布负责人 做签字。
- 不得把聊天结论当成正式 Gate 结论。
- 不得在缺少上游输入时硬推进下游阶段。
