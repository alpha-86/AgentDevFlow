...
- Gate 门控如果只停留在文档规则里，现实执行中很容易被绕过
...
| 启用方式 | 角色 | 一句话职责 | 定义文档 |
| --- | --- | --- | --- |
| 默认 | 团队负责人（Team Lead） | 负责团队节奏、流程完整性、跨角色协同和升级协调。**在 Claude Agent Team 中，Team Lead 是 human 用户本身，不是创建的 agent。** | [team-lead.md](./skills/shared/agents/team-lead.md) |
...
1. [Claude 接入](./docs/platforms/claude-code.md) - Claude 侧自举入口（**注意**：Team Lead 是 human 用户本身，不是创建的 agent）
2. [Codex 接入](./docs/platforms/codex.md) - Codex 侧主会话编排入口
3. [插件入口](./plugins/agentdevflow/README.md) - AgentDevFlow 插件在 Codex 中的接入方式
